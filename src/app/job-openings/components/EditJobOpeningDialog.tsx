...
export function EditJobOpeningDialog({
  ...
  }, [openingToEdit, isOpen, form.reset, allExistingContacts]); 
  ...
  useEffect(() => {
    if (openingToEdit) {
      ...
      const initialEmailDate = openingToEdit.initial_email_date ? new Date(openingToEdit.initial_email_date) : null;
      if (initialEmailDate && isValid(initialEmailDate) && emailSentStatuses.includes(openingToEdit.status as any)) {
        occurredEvents.push({
      ...
      const sentFollowUps = (openingToEdit.followUps || [])
