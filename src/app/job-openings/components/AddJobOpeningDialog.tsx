...
export function AddJobOpeningDialog({
  ...
  const onSubmit = async (values: AddJobOpeningFormValues) => {
    ...
    if (jobOpeningsCount >= limits.jobOpenings) {
      limitMessage = `You have reached the limit of ${limits.jobOpenings} job openings.`;
      if (isInGracePeriod) {
        limitMessage = `Your premium plan has expired. You've reached the Free Tier limit of ${limits.jobOpenings} job openings.`;
      }
    ...
    if (isCreatingNewCompany && companiesCount >= limits.companies) {
      limitMessage = `Cannot create new company "${values.companyName}" as you have reached the limit of ${limits.companies} companies.`;
       if (isInGracePeriod) {
        limitMessage = `Your premium plan has expired. Cannot create new company "${values.companyName}" as you've reached the Free Tier limit of ${limits.companies} companies.`;
      }
    ...
    if (numPotentiallyNewContacts > 0 && (contactsCount + numPotentiallyNewContacts) > limits.contacts) {
      limitMessage = `Adding ${numPotentiallyNewContacts} new contact(s) would exceed your limit of ${limits.contacts} contacts.`;
       if (isInGracePeriod) {
        limitMessage = `Your premium plan has expired. Adding ${numPotentiallyNewContacts} new contact(s) would exceed your Free Tier limit of ${limits.contacts} contacts.`;
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-3xl shadow-xl">
        <DialogHeader>
          <DialogTitle className="font-headline">Add New Job Opening</DialogTitle>
          <DialogDescription>
            Enter the details of the job opening and associated contacts.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
                            ...
                            <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                                ...
                                <CommandInput
                                    ...
                                    onValueChange={(searchValue) => {
                                      ...
                                      form.setValue(`contacts.${index}.linkedin_url`, '');
                                      form.setValue(`contacts.${index}.phone`, '');
                                    }}
                                    onKeyDown={(e) => {
                                      if (e.key === 'Enter' && !e.nativeEvent.isComposing) {
                                        e.preventDefault();
               ...
               <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="https://example.com/job/123" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
