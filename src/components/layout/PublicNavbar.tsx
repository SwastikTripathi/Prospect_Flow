...
// User type removed as it will come from AuthContext
...
export function PublicNavbar({ activeLink }: PublicNavbarProps) {
  ...
  const { user, isLoadingAuth: isLoadingAuthContext } = useAuth(); // Use AuthContext
  ...
  useEffect(() => {
    ...
    // No direct Supabase auth calls needed here anymore, rely on AuthContext
  }, []);

  const handleSignOut = async () => {
    // setIsLoadingAuth(true); // Context handles loading state
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 max-w-screen-2xl items-center justify-between mx-auto px-[5vw] md:px-[10vw]">
        ...
        <nav className="flex items-center space-x-0.5 sm:space-x-1">
          ...
          {isLoadingAuthContext ? ( // Use loading state from AuthContext
